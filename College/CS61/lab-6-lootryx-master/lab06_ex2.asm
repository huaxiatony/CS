...
; Parameters: none
; Postcondition: This subroutine converts binary number inpuuted by user into a decimal number
...
DO_WHILE
  ...
  OUT
  NOT	R3,R0			; 
  ADD	R3,R3,#1		; The process of R0 converted to 2's complement being sent to Register 3
  ADD	R4,R3,R5		; add the value inside Register 3 [2's complement of user input] and Register 5 [ascii value of one] to Register 4
  BRz	ADD_TOTAL		; If the result is zero, send to branch ADD_TOTAL
  ADD	R1,R1,#-1		; If not, then decrement the counter [16]
  BRp	DO_WHILE		; If the counter is not 0, then iterate through loop again
  BRnz	END_01			; Else send to branch END_01
ADD_TOTAL
  LD	R4,DEC_ONE		; Load the decimal value of one
  ADD	R5,R1,#-1		; Decrement the value of R1 into R5
  BRnz	END_02			; If negative or zero, send to branch END_02
ADD_LOOP
...
WHILE_LOOP
  AND	R5,R2,R6			; send to R5 the ANDed value of R2 and R6
  BRn	ONE_BIT				; If negative, send to branch ONE_BIT
  LD	R0,ASCII_0			; load value of ascii zero into R0
  OUT					; output ascii 0
  ADD	R2,R2,R2			; add the value of R2 and R2 into R2
  ADD	R4,R4,#-1			; decrement the value of R4
  ADD	R3,R3,#-1			; decrement the value of R3
  BRz	END_LOOP			; if zero, send to branch END_LOOP
  ADD	R4,R4,#0			; check for BR nzp condition again
  BRp	WHILE_LOOP			; If pos, iterate through loop again
  LD	R0,ASCII_SPACE			; else load up space char ascii and output to console. 
  OUT
  LD	R4,COUNT_4			; load 4 count to R4
...
COUNT_4			.FILL #4	; Pineapple
COUNT_16_3400		.FILL #16	; Mango
BIT_MASK		.FILL x8000	; Mask
ASCII_0			.FILL x30
ASCII_1			.FILL x31
ASCII_SPACE		.FILL x20

B_CHAR			.STRINGZ "b"	; Peach
NEWLINE			.STRINGZ "\n"
