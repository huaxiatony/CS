...
int main(int argc, char* argv[]) {
  ...
  for(;;) {
    ...
    if(cmd == "insert") {
      ...
      // inorder, node height
      // t.dotty();
      if(mode == "height") t.mode_height();
      else if(mode == "preorder") t.mode_pre();
      else if(mode == "inorder") t.mode_in();
      else if(mode == "postorder") t.mode_post();
      else {
        cout << mode << " was not found, try again." << endl;
        t.dotty();
      }
    }
