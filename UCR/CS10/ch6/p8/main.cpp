...
int convertPosition(char boardPosition) {
    return static_cast<int>(boardPosition - 'a');
}
...
bool validPlacement(const vector<char>  &gameBoard, int positionIndex) {
    int v = gameBoard.size();
    if(positionIndex > v){
        return false;
    }
    else if(gameBoard.at(positionIndex) == 'X' || gameBoard.at(positionIndex) == 'O'){ 
        return false;
    }
    return true;
}
...
bool gameWon(const vector<char> &gameBoard) {
    ...
    for(int i = 0; i < v; ++i){
        if((gameBoard.at(i) == gameBoard.at(i+1)) && gameBoard.at(i+1) == gameBoard.at(i+2)){ 
            return true;
        }
        i += 2;
    }
    for(int j = 0; j < 3; ++j){
        if((gameBoard.at(j) == gameBoard.at(j+3)) && gameBoard.at(j+3) == gameBoard.at(j+6)){ 
            return true;
        }
    }
...
int main() {
    ...
    while(!boardFull(gameBoard) && !gameWon(gameBoard)){
        ...
        if(validPlacement(gameBoard,curPlay)){
            if(whosTurn == PLAYER1){
